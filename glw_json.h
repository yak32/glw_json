...
inline const char* search_obj_end(const char* in, char open_brace, char close_brace){
	int opened = 1;
	while(*in) {
		if (*in == close_brace) --opened;
		else if (*in == open_brace) ++opened;
		if (!opened) return in+1; // success - we found closing
		++in;
	}
...
inline const char* load_props(const char* in, prop_map& props, size_t& props_size) {
	...
	while (*in) {
		...
		if(props_size >= MAX_JSON_PROPS-1) return in-1; // not enough space - error
		props[props_size++] = p;
...
inline bool operator<(const string_ref& s1, const string_ref& s2){
	...
	while(i<s1.len && i<s2.len){
		...
		if (s2.str[i]<s1.str[i]) return false;
		++i;
	}
...
inline const char* load(const char* str, size_t len, bool& v){
	...
	else if (len == 5 && !strncmp(str, "false", len)) v = false;
	else return str;
	return str + len;
}
...
template <typename T, typename A> const char* load(const char* in, size_t len, std::vector<T, A>& t) {
...
const char* load_container(const char* in, size_t len, containter_type& t) {
	...
	do{
		...
		if (load(in, len, v) != in + len) return in-1;
		in += len;
		it = v;
		if (*in != ','){
			if (*in != ']') return in-1;
...
template <typename c, typename tr, typename a> void save(std::ostream& out, std::basic_string<c, tr, a>& t, int tabs){
	out << quote << t << quote;
}
...
bool save(std::ostream& out, std::vector<T,A>& t, int tabs){
